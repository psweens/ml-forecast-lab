...
def peak_weighted_mae(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    ...
    The weight is ``1 + clip((y_true - median) / (p90 - median), 0, 9)``: an
    interval at the median weighs 1, a tall peak weighs up to 10, and the cap
    stops a single freak spike from dominating. On a flat / degenerate target
    (``p90 == median``) it falls back to plain MAE.
...
def pinball_q90(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """Upper-quantile (0.9) pinball loss — under-shooting a value costs 9× more
