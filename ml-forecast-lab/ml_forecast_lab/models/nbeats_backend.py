"""
PyTorch N-BEATS forecasting model backend for ML Forecast Lab.

Implements a Generic N-BEATS architecture with doubly-residual stacking,
...
try:
    ...
    warnings.warn(
        "PyTorch is not installed. NBeatsModel will not be functional.",
        ImportWarning,
    )
...
class _NBeatsBlock(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, n_horizons: int,
                 n_fc_layers: int = 4):
        super().__init__()
        layers = []
    ...
    def forward(self, x):
        # x: (batch, input_size) — flattened
        h = self.fc_stack(x)          # (batch, hidden_size)
        backcast = self.backcast_proj(h)  # (batch, input_size)
...
class NBeatsModel(ForecastModel):
    """
    PyTorch N-BEATS time-series forecasting model.

    Uses generic N-BEATS with doubly-residual stacking, AdamW optimiser
    ...
    def __init__(
        ...
        self._model: Optional[_NBeatsNet] = None
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray, **kwargs: Any) -> Dict[str, Any]:
        """Train N-BEATS with PyTorch autograd and best-model checkpointing."""
        ...
        self._model = _NBeatsNet(
            ...
            self.n_stacks, self.blocks_per_stack, self.n_fc_layers,
    ...
    def export_onnx(self, path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Exported N-BEATS to ONNX: {path}")
            return True
    ...
    def get_params(self) -> Dict[str, Any]:
        return deepcopy({
            ...
            "blocks_per_stack": self.blocks_per_stack, "n_fc_layers": self.n_fc_layers,
    ...
    def load(self, path: str) -> None:
        ...
        self._is_fitted = True
        logger.info(f"Loaded N-BEATS model from {path}")
