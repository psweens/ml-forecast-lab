"""
PyTorch Crossformer forecasting model backend for ML Forecast Lab.

Implements the Crossformer architecture with segment embedding,
temporal self-attention within each channel, and cross-variable
attention across channels. Uses AdamW optimisation, CosineAnnealingLR
scheduling, best-model checkpointing, and multi-horizon output.
"""
...
try:
    ...
    warnings.warn(
        "PyTorch is not installed. CrossformerModel will not be functional.",
        ImportWarning,
    )


class _CrossformerNet(nn.Module):
    """Crossformer: segment embedding + temporal attention + cross-variable attention."""
    ...
    def __init__(self, seq_len: int, n_channels: int, seg_len: int = 6,
        ...
        self.padded_len = self.n_segs * seg_len

        # Segment embedding: Linear(seg_len, d_model) applied per channel
        self.seg_embed = nn.Linear(seg_len, d_model)

        # Positional embedding for segments
        self.pos_embed = nn.Parameter(torch.randn(1, 1, self.n_segs, d_model) * 0.02)

        # Temporal attention: TransformerEncoder per channel (over segments)
        ...
        # Cross-variable attention: TransformerEncoder across channels
        ...
        self.cross_encoder = nn.TransformerEncoder(cross_layer, num_layers=n_layers)

        self.layer_norm = nn.LayerNorm(d_model)

        # Head: flatten (batch, n_channels, d_model) -> (batch, n_channels*d_model) -> output
    ...
    def forward(self, x):
        ...
        # Reshape to segments: (batch, n_segs, seg_len, n_channels)
        x = x.reshape(batch_size, self.n_segs, self.seg_len, self.n_channels)

        # Permute to (batch, n_channels, n_segs, seg_len)
        x = x.permute(0, 3, 1, 2)

        # Embed segments: Linear(seg_len, d_model) -> (batch, n_channels, n_segs, d_model)
        x = self.seg_embed(x)

        # Add positional embedding (broadcast over batch and n_channels)
        x = x + self.pos_embed
        ...
        # Cross-variable attention across channels
        x = self.cross_encoder(x)  # (batch, n_channels, d_model)
        x = self.layer_norm(x)
...
class CrossformerModel(ForecastModel):
    """
    PyTorch Crossformer time-series forecasting model.

    Segments the input sequence, applies temporal self-attention within
    each channel and cross-variable attention across channels. Uses AdamW
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray, **kwargs: Any) -> Dict[str, Any]:
        """Train Crossformer with PyTorch autograd and best-model checkpointing."""
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, len(X_tr_t), self.batch_size):
                ...
                # the same scale (the weighted `loss` above is used for
                # backprop only).
    ...
    def load(self, path: str) -> None:
        ...
        self._is_fitted = True
        logger.info(f"Loaded Crossformer model from {path}")
