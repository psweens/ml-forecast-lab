...
Implements TiDE (Time-series Dense Encoder) with residual MLP
encoder-decoder architecture, AdamW optimisation, CosineAnnealingLR
...
class _ResidualBlock(nn.Module):
    """Residual block: Linear -> ReLU -> Dropout + skip connection + LayerNorm."""
...
class _TiDENet(nn.Module):
    """TiDE: Time-series Dense Encoder-Decoder."""

    def __init__(self, seq_len: int, n_channels: int, hidden_size: int,
                 encoder_layers: int, decoder_layers: int, dropout: float,
                 n_horizons: int = 1):
        super().__init__()
        self.n_horizons = n_horizons
        flat_input_size = seq_len * n_channels

        # Input projection
        self.input_proj = _ResidualBlock(flat_input_size, hidden_size, dropout)
        ...
        # Output projection
        self.output_proj = nn.Linear(hidden_size, n_horizons)
    ...
    def forward(self, x):
        ...
        h = self.input_proj(x_flat)              # (batch, hidden_size)
        h = self.encoder(h)                      # (batch, hidden_size)
        h = self.decoder(h)                      # (batch, hidden_size)
        out = self.output_proj(h)                # (batch, n_horizons)
...
class TiDEModel(ForecastModel):
    ...
    Uses TiDE (Time-series Dense Encoder) with residual MLP blocks,
    AdamW optimiser with CosineAnnealingLR, and best-model checkpointing
    on validation loss. Supports multi-horizon output.
    """
    ...
    def export_onnx(self, path: str) -> bool:
        ...
        if not self.is_fitted or self._model is None:
            return False
        try:
            dummy = torch.randn(1, self.sequence_length or 48, self._input_size or 1)
            torch.onnx.export(self._model, dummy, path, input_names=["input"], output_names=["output"])
            logger.info(f"Exported TiDE to ONNX: {path}")
            return True
