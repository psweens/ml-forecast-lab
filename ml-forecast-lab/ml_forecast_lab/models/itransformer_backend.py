"""
PyTorch iTransformer forecasting model backend for ML Forecast Lab.

Implements the iTransformer architecture where each variable's full time
series is treated as one token and attention is applied across variables.
Uses AdamW optimisation, CosineAnnealingLR scheduling, best-model
checkpointing, and multi-horizon output via a shared encoder and
multi-output head.
"""
...
try:
    ...
    warnings.warn(
        "PyTorch is not installed. iTransformerModel will not be functional.",
        ImportWarning,
    )


class _iTransformerNet(nn.Module):
    """iTransformer: each channel's full sequence is one token; attention across channels."""

    def __init__(self, seq_len: int, n_channels: int, d_model: int = 32,
        ...
        # Transformer encoder across channel tokens
        ...
        # Flatten (batch, n_channels, d_model) -> (batch, n_channels*d_model) -> head
    ...
    def forward(self, x):
        ...
        # Transformer encoder across channel tokens
        x = self.encoder(x)  # (batch, n_channels, d_model)
        x = self.layer_norm(x)
...
class iTransformerModel(ForecastModel):
    """
    PyTorch iTransformer time-series forecasting model.

    Each variable's full time series is treated as a single token and
    self-attention is applied across variables (channels). Uses AdamW
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray, **kwargs: Any) -> Dict[str, Any]:
        """Train iTransformer with PyTorch autograd and best-model checkpointing."""
    ...
    def load(self, path: str) -> None:
        ...
        self._is_fitted = True
        logger.info(f"Loaded iTransformer model from {path}")
