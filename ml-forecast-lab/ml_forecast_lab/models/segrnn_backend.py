"""
SegRNN forecasting model backend for ML Forecast Lab.

Implements the segment-wise recurrent architecture from:
  "SegRNN: Segment Recurrent Neural Network for Long-Term Time Series
  Forecasting" (Lin et al., 2023, https://arxiv.org/abs/2308.11200)

Instead of step-by-step recurrence (the wired `lstm` / `gru` backends),
the input window is partitioned into segments; each segment becomes one
GRU token, cutting recurrence steps by the segment length. The forecast
is produced by the paper's PMF (parallel multi-step forecasting)
decoding: one learnable position embedding per output segment, all
decoded in parallel from the encoder's final state — no error-
accumulating autoregressive rollout.

House adaptations (documented deviations from the reference code):
- The paper is channel-independent with a per-channel output and a
  channel-identity embedding. MLFL forecasts a single target from a
  multi-channel window, so the segment token embeds all channels at once
  and the channel embedding is dropped; the PMF positional decoding is
  unchanged. Covariate channels (future-known rows of the extended
  window included) reach the forecast through the segment tokens.
- The paper's fixed training recipe is replaced by the house kit shared
  by every neural backend: RevIN with past-only stats, AdamW + cosine
  annealing, composite-horizon loss, best-model checkpointing.
"""
...
try:
    ...
    warnings.warn(
        "PyTorch is not installed. SegRNNModel will not be functional.",
        ImportWarning,
    )


class _SegRNNNet(nn.Module):
    """SegRNN: segment embedding, GRU encoder, PMF parallel decoding."""

    def __init__(self, seq_len: int, n_channels: int, seg_len: int,
                 d_model: int, dropout: float, n_horizons: int = 1,
        ...
        # Clamp seg_len so at least one complete segment fits
        seg_len = max(1, min(seg_len, seq_len))
        self.seg_len = seg_len

        # Number of complete input segments; the forward keeps the most
        # recent seg_num_x * seg_len timesteps (same clamp idiom as
        # SparseTSF's cross-period reshape).
        self.seg_num_x = seq_len // seg_len
        # Output segments cover the horizon; the tail of the last segment
        # is sliced off when n_horizons is not a multiple of seg_len.
        self.seg_num_y = max(1, -(-n_horizons // seg_len))

        # Segment embedding. The paper embeds each channel's seg_len values
        # independently; here one token embeds the segment across all
        # channels (single-target adaptation, see module docstring).
        self.value_embedding = nn.Sequential(
            nn.Linear(seg_len * n_channels, d_model),
            nn.ReLU(),
        )
        self.rnn = nn.GRU(input_size=d_model, hidden_size=d_model,
                          num_layers=1, batch_first=True)
        # PMF decoding: one learnable position embedding per output segment,
        # decoded in parallel from the replicated encoder state. The paper's
        # channel-identity half of this embedding is dropped (single target).
        self.pos_emb = nn.Parameter(torch.randn(self.seg_num_y, d_model))
    ...
    def forward(self, x):
        ...
        # Keep the most recent complete segments
        effective_len = self.seg_num_x * self.seg_len
        x = x[:, x.size(1) - effective_len:, :]

        # Segment and embed: (batch, seg_num_x, seg_len * n_channels) -> tokens
        x = x.reshape(batch_size, self.seg_num_x, self.seg_len, self.n_channels)
        x = x.reshape(batch_size, self.seg_num_x, self.seg_len * self.n_channels)
        tokens = self.value_embedding(x)  # (batch, seg_num_x, d_model)

        # Encode: final hidden state summarises the whole window
        _, hn = self.rnn(tokens)  # (1, batch, d_model)

        # PMF decode: replicate the encoder state per output segment and
        # feed each segment's position embedding as a single-step input.
        # Both reshapes are batch-major / segment-minor so state and
        # position embedding stay paired.
        m = self.seg_num_y
        pos = self.pos_emb.unsqueeze(0).expand(batch_size, m, self.d_model)
        pos = pos.reshape(batch_size * m, 1, self.d_model)
        h0 = hn.repeat(1, 1, m).view(1, batch_size * m, self.d_model)
        _, hy = self.rnn(pos, h0)  # (1, batch * m, d_model)

        y = self.predict(hy.squeeze(0))  # (batch * m, seg_len)
        y = y.view(batch_size, m * self.seg_len)[:, :self.n_horizons]
...
class SegRNNModel(ForecastModel):
    """
    SegRNN time-series forecasting model.

    Segment-wise GRU encoding with parallel multi-step (PMF) decoding.
    Far fewer recurrence steps than the step-wise LSTM/GRU backends,
    which is what makes RNN accuracy competitive again at long horizons.
    ...
    def __init__(
        ...
        self._model: Optional[_SegRNNNet] = None
    ...
    def _build_model(self, seq_len: int, n_channels: int,
        ...
        return _SegRNNNet(
            seq_len, n_channels, self.seg_len, self.d_model, self.dropout,
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray, **kwargs: Any) -> Dict[str, Any]:
        """Train SegRNN with PyTorch autograd and best-model checkpointing."""
    ...
    def get_params(self) -> Dict[str, Any]:
        return deepcopy({
            "seg_len": self.seg_len, "d_model": self.d_model,
    ...
    def set_params(self, **kwargs: Any) -> None:
        valid = {"seg_len", "d_model", "dropout", "learning_rate",
    ...
    def save(self, path: str) -> None:
        ...
        logger.info(f"Saved SegRNN model to {path}")
    ...
    def load(self, path: str) -> None:
        ...
        if self._seq_len is not None and self._n_channels is not None:
            ...
            self._model.eval()
        self._is_fitted = True
        logger.info(f"Loaded SegRNN model from {path}")
