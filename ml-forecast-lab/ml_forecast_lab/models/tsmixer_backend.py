"""
PyTorch TSMixer forecasting model backend for ML Forecast Lab.

Implements a TSMixer architecture with alternating time-mixing and
feature-mixing MLP blocks, AdamW optimisation, CosineAnnealingLR
scheduling, and best-model checkpointing. Supports multi-horizon
output via a shared encoder and multi-output head.
"""
...
try:
    ...
    warnings.warn(
        "PyTorch is not installed. TSMixerModel will not be functional.",
        ImportWarning,
    )


class _MixerLayer(nn.Module):
    """Single TSMixer block: time-mixing then feature-mixing with residuals."""

    def __init__(self, seq_len: int, n_channels: int, hidden: int, dropout: float):
        super().__init__()
        # Time-mixing MLP (applied along the time axis)
        self.time_norm = nn.LayerNorm(n_channels)
        self.time_fc1 = nn.Linear(seq_len, hidden)
        self.time_fc2 = nn.Linear(hidden, seq_len)
        self.time_drop = nn.Dropout(dropout)

        # Feature-mixing MLP (applied along the channel axis)
        self.feat_norm = nn.LayerNorm(n_channels)
        self.feat_fc1 = nn.Linear(n_channels, hidden)
        self.feat_fc2 = nn.Linear(hidden, n_channels)
        self.feat_drop = nn.Dropout(dropout)

    def forward(self, x):
        # x: (batch, seq_len, n_channels)

        # Time-mixing: transpose to (batch, n_channels, seq_len), mix, transpose back
        residual = x
        h = self.time_norm(x)
        h = h.transpose(1, 2)  # (batch, n_channels, seq_len)
        h = self.time_fc1(h)   # (batch, n_channels, hidden)
        h = F.relu(h)
        h = self.time_drop(h)
        h = self.time_fc2(h)   # (batch, n_channels, seq_len)
        h = h.transpose(1, 2)  # (batch, seq_len, n_channels)
        x = residual + h

        # Feature-mixing
        residual = x
        h = self.feat_norm(x)
        h = self.feat_fc1(h)   # (batch, seq_len, hidden)
        h = F.relu(h)
        h = self.feat_drop(h)
        h = self.feat_fc2(h)   # (batch, seq_len, n_channels)
        x = residual + h

        return x


class _TSMixerNet(nn.Module):
    """TSMixer: stack of MixerLayers followed by flatten and linear head."""
...
class TSMixerModel(ForecastModel):
    """
    PyTorch TSMixer time-series forecasting model.

    Uses alternating time-mixing and feature-mixing MLP blocks,
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray, **kwargs: Any) -> Dict[str, Any]:
        """Train TSMixer with PyTorch autograd and best-model checkpointing."""
    ...
    def export_onnx(self, path: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Exported TSMixer to ONNX: {path}")
            return True
    ...
    def get_params(self) -> Dict[str, Any]:
        return deepcopy({
            "n_mixer_layers": self.n_mixer_layers, "hidden": self.hidden,
    ...
    def load(self, path: str) -> None:
        ...
        self._is_fitted = True
        logger.info(f"Loaded TSMixer model from {path}")
