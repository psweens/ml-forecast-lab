"""
PyTorch ModernTCN forecasting model backend for ML Forecast Lab.

Implements the ModernTCN architecture (Luo & Wang 2024, ICLR,
https://openreview.net/forum?id=vpJMJerXHU): a modernised pure-convolution
backbone that patchifies each variable independently, then stacks blocks of

- a large-kernel depthwise convolution over the patch axis (temporal
  mixing with a receptive field competitive with attention), with a
  parallel small-kernel branch for local detail,
- ConvFFN1: grouped pointwise convolution mixing the feature dimension
  *within* each variable (channel-independent feature mixing),
- ConvFFN2: grouped pointwise convolution mixing *across* variables per
  feature dimension (cross-variable mixing),

each with residual connections. The decomposition of temporal /
per-variable / cross-variable mixing into three cheap convolutions is
what lets it match transformer accuracy at a fraction of the compute —
a good fit for Pi-class hardware.

Uses AdamW optimisation, CosineAnnealingLR scheduling, and best-model
checkpointing. Supports multi-horizon output via a shared backbone and
multi-output head.
"""
...
try:
    ...
    warnings.warn(
        "PyTorch is not installed. ModernTCNModel will not be functional.",
        ImportWarning,
    )


class _ModernTCNBlock(nn.Module):
    """One ModernTCN block: large-kernel DWConv + ConvFFN1 + ConvFFN2.

    Operates on a (batch, n_vars, d_model, n_patches) tensor. Grouped
    1x1 convolutions implement the paper's two-stage feature mixing:
    ConvFFN1 groups by variable (mixes d_model within each variable),
    ConvFFN2 groups by feature (mixes variables within each feature dim).
    """

    def __init__(self, n_vars: int, d_model: int, large_kernel: int,
                 small_kernel: int, ffn_ratio: int, dropout: float):
        super().__init__()
        self.n_vars = n_vars
        self.d_model = d_model
        dw_channels = n_vars * d_model

        # Depthwise temporal mixing: one filter per (variable, feature)
        # pair. The parallel small-kernel branch is the paper's
        # structural re-parameterisation companion — kept as an explicit
        # parallel branch here (no inference-time kernel merging; at
        # Pi-scale widths the saving would be negligible).
        self.dw_large = nn.Conv1d(
            dw_channels, dw_channels, kernel_size=large_kernel,
            padding=large_kernel // 2, groups=dw_channels,
        )
        self.dw_small = nn.Conv1d(
            dw_channels, dw_channels, kernel_size=small_kernel,
            padding=small_kernel // 2, groups=dw_channels,
        )
        self.bn = nn.BatchNorm1d(dw_channels)

        # ConvFFN1: per-variable feature mixing (groups = n_vars).
        hidden1 = d_model * ffn_ratio
        self.ffn1_pw1 = nn.Conv1d(
            n_vars * d_model, n_vars * hidden1, kernel_size=1, groups=n_vars,
        )
        self.ffn1_pw2 = nn.Conv1d(
            n_vars * hidden1, n_vars * d_model, kernel_size=1, groups=n_vars,
        )

        # ConvFFN2: cross-variable mixing per feature dim (groups = d_model,
        # applied on the (d_model, n_vars)-flattened layout).
        hidden2 = n_vars * ffn_ratio
        self.ffn2_pw1 = nn.Conv1d(
            d_model * n_vars, d_model * hidden2, kernel_size=1, groups=d_model,
        )
        self.ffn2_pw2 = nn.Conv1d(
            d_model * hidden2, d_model * n_vars, kernel_size=1, groups=d_model,
        )
    ...
    def forward(self, x):
        ...
        # ---- Temporal mixing (DWConv, variable-major layout) ----
        y = x.reshape(batch, m * d, n)
        y = self.dw_large(y) + self.dw_small(y)
        y = self.bn(y)
        x = x + y.reshape(batch, m, d, n)  # residual

        # ---- ConvFFN1: feature mixing within each variable ----
        y = x.reshape(batch, m * d, n)
        y = self.ffn1_pw2(self.dropout(self.act(self.ffn1_pw1(y))))
        x = x + y.reshape(batch, m, d, n)  # residual

        # ---- ConvFFN2: cross-variable mixing per feature dim ----
        y = x.permute(0, 2, 1, 3).reshape(batch, d * m, n)
        y = self.ffn2_pw2(self.dropout(self.act(self.ffn2_pw1(y))))
        y = y.reshape(batch, d, m, n).permute(0, 2, 1, 3)
        x = x + y  # residual
...
class _ModernTCNNet(nn.Module):
    """ModernTCN: patchify stem + DWConv/ConvFFN blocks + flatten head."""

    def __init__(self, seq_len: int, n_channels: int, patch_len: int,
                 patch_stride: int, d_model: int, large_kernel: int,
                 small_kernel: int, ffn_ratio: int, n_blocks: int,
        ...
        if patch_len > seq_len:
            patch_len = max(1, seq_len)
        patch_stride = max(1, min(patch_stride, patch_len))
        self.patch_len = patch_len
        self.patch_stride = patch_stride
        self.n_patches = (seq_len - patch_len) // patch_stride + 1

        # Patchify stem: variable-independent strided conv embedding —
        # each variable's series becomes (d_model, n_patches).
        self.stem = nn.Conv1d(1, d_model, kernel_size=patch_len, stride=patch_stride)

        self.blocks = nn.ModuleList([
            _ModernTCNBlock(
                n_vars=n_channels, d_model=d_model,
        ...
        # Output head: flatten the full (n_vars, d_model, n_patches)
        # representation and project to horizons. The flatten head sees
        # every patch — including future-block patches in extended-window
        # mode, which carry the known-future covariate values — so no
        # separate future-aux head is needed (same rationale as the
        # SparseTSF / CNN flatten heads).
        self.head = nn.Linear(n_channels * d_model * self.n_patches, n_horizons)
    ...
    def forward(self, x):
        ...
        if self.revin is not None:
            # PF1: past-only stats in extended-window mode — the zeroed
            # future target positions must not bias the per-window mean.
            x = self.revin.normalize(x, past_window_size=self.past_window_size)

        # Channel-first: (batch, n_channels, seq_len)
        x = x.transpose(1, 2)

        # Variable-independent patchify: (batch * n_channels, 1, seq_len)
        # -> (batch * n_channels, d_model, n_patches)
        x = x.reshape(batch_size * self.n_channels, 1, self.seq_len)
        x = self.stem(x)
        x = x.reshape(batch_size, self.n_channels, self.d_model, -1)
...
class ModernTCNModel(ForecastModel):
    """
    PyTorch ModernTCN time-series forecasting model.

    Modernised pure-convolution backbone (Luo & Wang 2024): per-variable
    patchify stem, large-kernel depthwise temporal convolution, and
    grouped pointwise convolutions for per-variable and cross-variable
    feature mixing. Uses AdamW with CosineAnnealingLR, best-model
    ...
    def __init__(
        ...
        self._model: Optional[_ModernTCNNet] = None
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray, **kwargs: Any) -> Dict[str, Any]:
        """Train ModernTCN with PyTorch autograd and best-model checkpointing."""
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, len(X_tr_t), self.batch_size):
                ...
                # BatchNorm in the DWConv stage needs >= 2 samples to
                # compute batch statistics — skip a trailing 1-sample
                # batch rather than crash (it rejoins via the next epoch's
                # reshuffle).
                if X_batch.size(0) < 2:
                    continue
    ...
    def get_params(self) -> Dict[str, Any]:
        return deepcopy({
            ...
            "d_model": self.d_model, "large_kernel": self.large_kernel,
            "small_kernel": self.small_kernel, "ffn_ratio": self.ffn_ratio,
    ...
    def set_params(self, **kwargs: Any) -> None:
        valid = {"patch_len", "patch_stride", "d_model", "large_kernel",
                 "small_kernel", "ffn_ratio", "n_blocks", "dropout",
    ...
    def save(self, path: str) -> None:
        ...
        logger.info(f"Saved ModernTCN model to {path}")
    ...
    def load(self, path: str) -> None:
        ...
        self._is_fitted = True
        logger.info(f"Loaded ModernTCN model from {path}")
